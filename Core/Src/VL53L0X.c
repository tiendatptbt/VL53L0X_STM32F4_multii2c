...
void VL53L0X_writeReg(struct VL53L0X* dev, uint8_t reg, uint8_t value)
{
	...
	buf[0] = reg;
	buf[1] = value;
	i2c_write(dev->hi2c,dev->address, buf, 2);
}
...
void VL53L0X_writeReg16Bit(struct VL53L0X* dev, uint8_t reg, uint16_t value)
{
	...
	buf[2] = (uint8_t) (value & 0xFF);
	 i2c_write(dev->hi2c,dev->address, buf, 3);
}
...
void VL53L0X_writeReg32Bit(struct VL53L0X* dev, uint8_t reg, uint32_t value)
{
	...
	i2c_write(dev->hi2c,dev->address, buf, 5);
}
...
{
  uint8_t value;
  i2c_write(dev->hi2c,dev->address, &reg, 1);
  i2c_read(dev->hi2c,dev->address, &value, 1);
  return value;
}
...
uint16_t VL53L0X_readReg16Bit(struct VL53L0X* dev, uint8_t reg)
{
  ...
  uint8_t buf[2];
  i2c_write(dev->hi2c,dev->address, &reg, 1);
  i2c_read(dev->hi2c,dev->address, buf, 2);
...
uint32_t VL53L0X_readReg32Bit(struct VL53L0X* dev, uint8_t reg)
{
  ...
  uint8_t buf[4];
  i2c_write(dev->hi2c,dev->address, &reg, 1);
  i2c_read(dev->hi2c,dev->address, buf, 4);
...
{
	i2c_write(dev->hi2c,dev->address, &reg, 1);
	i2c_write(dev->hi2c,dev->address, src, count);
}
...
{
	i2c_write(dev->hi2c,dev->address, &reg, 1);
	i2c_read(dev->hi2c,dev->address, dst, count);
}
